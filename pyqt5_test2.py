...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # 创建一个 QVBoxLayout 和 QHBoxLayout
        main_layout = QVBoxLayout()
        side_layout = QVBoxLayout()
        layout = QHBoxLayout()

        # 创建 QLabel 用于显示视频，并设置大小
        self.label = QLabel()
        self.label.setFixedSize(600, 400)
        layout.addWidget(self.label)

        # 创建两个按钮
        self.button1 = QPushButton('开启摄像头')
        self.button2 = QPushButton('人物识别')
        ...
        # 将按钮的 clicked 信号连接到对应的槽函数
        self.button1.clicked.connect(self.openCamera)
        self.button1.clicked.connect(self.yolo)
        self.button3.clicked.connect(self.closeCamera)

        # 将 QVBoxLayout 设置为窗口的布局
