...
def calculate_distance(depth_image, bbox):
    depth_values = depth_image[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])]
    avg_depth = np.mean(depth_values)
    distance = 1 / avg_depth  # Calculate distance using depth value
    return distance
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # 对每帧图片进行检测detections per image
            ...
            if webcam:  # batch_size >= 1
                ...
                distance = calculate_distance(depth_image, det[:4])
            # 若为一个其他源(单个图片)
            else:
                # 获取属性
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():  # 对每个唯一的类别标签进行迭代
                    ...
                    s += f"Distance: {distance:.2f} meters, "  # Add distance to string
                # 保存结果Write results
