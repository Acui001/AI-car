...
speed_value = bytearray([0x01])  # 给值，但是单独给并不导致车动，需要给舵机打直send_str，
speed_value_zero = bytearray([0x02])
...
async def communicate_device(client):
    ...
    global person_exist
    if person_exist:
        await client.write_gatt_char(par_write_characteristic, speed_value, True)
    else:
        ...
        with torch.no_grad():
            if opt.update:
                for opt.weights in ['yolov7.pt']:
                    await asyncio.get_event_loop().run_in_executor(None, lambda: detect_async())
