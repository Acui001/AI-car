"""
没有蓝牙传输部分
这里是一个判断你在中间左右的一个程序,在一个基础上将区间划分的更加细腻，对情况的判断更加精确
并且判断你在哪边
"""
import argparse
import time
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # 对每帧图片进行检测detections per image
            ...
            visualize_image_segments(im0, segment_weights)  # 可视化分割区域 # 没有也不怕，这段只是为了明显可视化的，你可以将他注释掉
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):  # 用于遍历和处理检测结果
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        position=get_anchor_box_interval(xyxy, image_width, segment_weights)
                        print("当前位置为:", position)
                        # 'Left_high', 'Left_mid', 'Left_low', 'Centre', 'Right_low', 'Right_mid', 'Right_high'
                        if position == 'Left_high':
                            # 处理 'left_high' 的情况
                            # 这里可以放置你想要执行的代码
                            # 调整方向的话给 Turn 按键赋值
                            print("处理 Left_high 的情况")
                        elif position == 'Left_mid':
                            # 处理 'left_mid' 的情况
                            # 这里可以放置你想要执行的代码
                            # 调整方向的话给 Turn 按键赋值
                            print("处理 Left_mid 的情况")
                        elif position == 'Left_low':
                            # 处理 'left_low' 的情况
                            # 这里可以放置你想要执行的代码
                            # 调整方向的话给 Turn 按键赋值
                            print("处理 Left_low 的情况")
                        elif position == 'Centre':
                            # 处理 'centre' 的情况
                            # 这里可以放置你想要执行的代码
                            # 调整方向的话给 Turn 按键赋值
                            print("处理 Centre 的情况")
                        elif position == 'Right_low':
                            # 处理 'right_low' 的情况
                            # 这里可以放置你想要执行的代码
                            # 调整方向的话给 Turn 按键赋值
                            print("处理 Right_low 的情况")
                        elif position == 'Right_mid':
                            # 处理 'right_mid' 的情况
                            # 这里可以放置你想要执行的代码
                            # 调整方向的话给 Turn 按键赋值
                            print("处理 Right_mid 的情况")
                        elif position == 'Right_high':
                            # 处理 'right_high' 的情况
                            # 这里可以放置你想要执行的代码
                            # 调整方向的话给 Turn 按键赋值
                            print("处理 Right_high 的情况")
