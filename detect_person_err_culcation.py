...
speed_value_zero = bytearray([0x02])  # 固定停车值
# ——————速度方向——————————
...
async def communicate_device(client):
    ...
    global person_exist, send_speed_direction
    if person_exist:  # 有人的是时候的运行逻辑

        await client.write_gatt_char(par_write_characteristic, send_speed_value, True)
        await client.write_gatt_char(par_write_characteristic, send_speed_direction, True)
    else:  # 没有人的时候的停车逻辑
        await client.write_gatt_char(par_write_characteristic, speed_value_zero, True)
        await client.write_gatt_char(par_write_characteristic, send_str, True)
...
def visualize_image_segments(image):
    ...
    # 在图像上绘制垂直线条分割三个不均匀部分
    cv2.line(image, (2 * segment_width, 0), (2 * segment_width, height), (0, 255, 0), 2)
    cv2.line(image, (3 * segment_width, 0), (3 * segment_width, height), (0, 255, 0), 2)


def detect_anchor_distance(anchor_box, image_width):
    anchor_center = (anchor_box[0] + anchor_box[2]) / 2  # 计算锚框的中心点 x 坐标
    segment_width = image_width // 5
    if anchor_center < 2 * segment_width:  # 锚框在视野的左侧1/3处
        distance = abs(anchor_center - 2 * segment_width)
        return distance
    elif anchor_center > 3 * segment_width:  # 锚框在视野的右侧1/3处
        distance = abs(anchor_center - 3 * segment_width)
        return -distance
    else:  # 锚框在视野的中间1/3处
        return 0
...
def detect(save_img=False):
    ...
    depth_frame = frames.get_depth_frame()  # depth是景深摄像头的数据，这里几乎把可以获取的数据已经获取了
    # ———————————————————————————————————获取摄像头数据部分———结束——————————————————————————————— #
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):  # 用于遍历和处理检测结果
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        distance = detect_anchor_distance(xyxy, image_width)
                        print("当前位置误差为:", distance)
                        if distance > 0:
                            # 处理 "Left" 的情况
                            # 这里可以放置你想要执行的代码
                            # 调整方向的话给 Turn按键赋值
                            # send_speed_direction已经在detect最前面声明了为全局变量了
                            send_speed_direction = send_left
                            print("检测到人在左边，正在左转中(:")
                        elif distance < 0:
                            # 处理 "Right" 的情况
                            # 这里可以放置你想要执行的代码
                            # 调整方向的话给 Turn按键赋值
                            send_speed_direction = send_right
