    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # 对每帧图片进行检测detections per image
            ...
            if "person" in s:
                person_exist = True
                if len(pred) == 1:
                    cv2.putText(im0, 'Only one person press "r" to lock Target', (w//2, h//2),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
                    if cv2.waitKey(1) & 0xFF == ord('r'):  # 检查 Enter 键是否被按下
                        # 在这里修改标签
                        # 例如，假设 `pred` 是一个包含标签的列表，你可以通过索引来修改标签值
                        pred[0] = 'Target'

                        break
                else:
                    cv2.putText(im0, 'keep only one person to lock Target', (w // 2, h // 2),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--classes', nargs='+', type=int, default=[0], help='filter by class: --class 0, or --class 0 2 3')  # 选中可以只显示特定的标签
