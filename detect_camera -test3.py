...
def get_distance(depth_frame, x, y,r):
    # 获取深度图像中指定位置的距离值
    x, y = int(x), int(y)
...
def mark_point_TxT(image,distance, x, y):
    ...
    cv2.putText(marked_image, str(distance)+"m", (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):  # 用于遍历和处理检测结果
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        r = min(xyxy[2] - xyxy[0],xyxy[3] - xyxy[0])
                        # 获取深度图像中指定位置的距离值
                        distance = get_distance(depth_frame, X, Y,r)
