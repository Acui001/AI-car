...
# ————————————————————————————————————启用蓝牙部分————开始—————————————————————————————————— #
# 定义(小车)BLE设备的MAC地址
device_mac = "98:DA:20:05:70:5D"  # 替换为你实际的设备MAC地址！！！！记得换
# 获取特征对象，你要改的是车的码
characteristic_uuid = "00001101-0000-1000-8000-00805f9b34fb"  # 替换为你实际的特征UUID！！！！！！记得换
# 定义要发送的消息
message = "GF_haven`t_dick"  # 根据你要求改吧
# 创建BLE设备对象
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # 对每帧图片进行检测detections per image
            ...
            if "bottle" in s:
                ...
                # 发送消息
                device.char_write(characteristic_uuid, message_bytes, wait_for_response=False)
                # ——————————————————————————蓝牙运行部分——————————————结束————————————————————#
                print("小车行为加在这里，但是别真加在print里面，这个只是用来测试的:)")
                # ——————————————————————小车行为(通信部分)加在这里———终止端———————————————————-#
...
if __name__ == '__main__':
    ...
    parser.add_argument('--classes', nargs='+', type=int, default=[39],
