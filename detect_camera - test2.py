...
#     distance_list = []
#     mid_pos = [(box[0] + box[2]) // 2, (box[1] + box[3]) // 2]  # Determine the center pixel position for depth indexing
#     min_val = min(abs(box[2] - box[0]), abs(box[3] - box[1]))  # Determine the depth search range
#
#     for i in range(randnum):
#         bias = random.randint(-min_val // 4, min_val // 4)
#         depth_value = depth_frame.get_distance(int(mid_pos[0] + bias), int(mid_pos[1] + bias))
#         cv2.circle(frame, (int(mid_pos[0] + bias), int(mid_pos[1] + bias)), 4, (255, 0, 0), -1)
#         if depth_value != 0:
#             distance_list.append(depth_value)
#
#     distance_list = np.array(distance_list)
#     distance_list = np.sort(distance_list)[randnum // 2 - randnum // 4:randnum // 2 + randnum // 4]  # Bubble sort + median filtering
...
def dectshow(img, boxs, depth_data):
    for box in boxs:
        box = torch.tensor(box)
        cv2.rectangle(img, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (0, 255, 0), 2)
        dist = get_mid_pos(img, box, depth_data, 24)
        label = str(box[-1].item()) + "EE{:.2f}".format(dist) + 'm'
        cv2.putText(img, label, ((int(box[2]/2)), int(box[1]+10)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # 对每帧图片进行检测detections per image
            ...
            if len(det):
                ...
                # 在每次识别标签时，向字典中添加点的信息和深度值
                for *xyxy, conf, cls in reversed(det):
                    dectshow(im0, [xyxy], depth_frame)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--classes', nargs='+', type=int, default=[0, 1, 2, 3, 4, 6, 8],
